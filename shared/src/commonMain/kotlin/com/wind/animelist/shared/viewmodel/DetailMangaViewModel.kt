...
class DetailMangaViewModel(val di: DI): BaseViewModel() {
    ...
    fun setManga(manga: Manga) {
        clientScope.launch {
            getCharacterInMangaUseCase.invoke(GetCharacterInMangaParam(manga.id)).let {
                MangaCharacter(it.successOr(emptyList())).let {mangaCharacter ->
                    if (mangaCharacter.list.isNotEmpty()) {
                        list.add(Title("Character"))
